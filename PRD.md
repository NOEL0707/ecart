...
Discount System works in the following ways - Every Every nth order gets a coupon code for x% discount. Discount codes can be applied at checkout.

The store also has two admin API's:

Generate a discount code if the condition above is satisfied.
Lists count of items purchased, revenue, discount codes and total discounts given.
